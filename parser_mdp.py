...
class Parser:
    ...
    def get_all_action(self):
        ...
        for action_name in all_actions:
            current_action = re.findall("^{action_name}(.*?)^endaction".format(action_name=action_name), self.txt,
                                        re.DOTALL | re.MULTILINE)[0]
            actions = re.findall("robot-at-x[0-9]*y[0-9]* robot-at-x[0-9]*y[0-9]* [0-9]*.[0-9]* [0-9]*.[0-9]*",
                                 current_action)
